...
const u_int16_t POPULATION_SIZE = 350;
const u_int8_t OPPONENT_POPULATION_SIZE = 70;
const u_int8_t PERFECT_ONE_GAMER_NUMBER = 6;
const u_int8_t PERFECT_ONE_GAMER_NUMBER_OPPONENT = 2;
...
unsigned int generate_perfect_hurdler(Player &player, Game &game)
{
    unsigned int solution = 0;
    uint8_t i = 0;
    int8_t cur_pos = player.hurdle_position;
    int8_t cur_stun = player.hurdle_stun;
    int randval;

    while (i < TURN_SIMULATED)
    {
        if (cur_stun > 0)
        {
            cur_stun -= 1;
            if (i < game.diving_sequence.size())
            {
                if (game.diving_sequence[i] == 'U')
                    randval = 0;
                else if (game.diving_sequence[i] == 'L')
                    randval = 1;
                else if (game.diving_sequence[i] == 'D')
                    randval = 2;
                else
                    randval = 3;
            }
            ...
            solution |= randval << (30 - i * 2);
        }
        else
        {
            if (cur_pos < 29 && game.hurdle_field[cur_pos + 1] == '#')
            {
                solution |= 0 << (30 - i * 2);
                cur_pos += 2;
            }
            else if (cur_pos < 28 && game.hurdle_field[cur_pos + 2] == '#')
            {
                solution |= 1 << (30 - i * 2);
                cur_pos += 1;
            }
            else if (cur_pos < 27 && game.hurdle_field[cur_pos + 3] == '#')
            {
                if (i < game.diving_sequence.size())
                {
                    if (game.diving_sequence[i] == 'U')
...
unsigned int generate_perfect_diver(Player &player, Game &game)
{
    unsigned int solution = 0;
    uint8_t i = 0;
    int randval;

    while (i < game.diving_sequence.size())
    {
        if (game.diving_sequence[i] == 'U')
            solution |= 0 << (30 - i * 2);
        else if (game.diving_sequence[i] == 'L')
...
std::array<Solution, OPPONENT_POPULATION_SIZE> generate_opponent_initial_population(int8_t turns_left, Player &player, Game &game)
{
    ...
    if (game.hurdle_field != "GAME_OVER")
    {
        for (int i = 0; i < PERFECT_ONE_GAMER_NUMBER_OPPONENT; ++i)
        {
            unsigned int solution = generate_perfect_hurdler(player, game);
            Player playerCopy = player;
            population[i].solution = solution;
            population[i].score = evaluate(solution, turns_left, playerCopy, game);
        }
        start += PERFECT_ONE_GAMER_NUMBER_OPPONENT;
    }
    if (game.diving_sequence != "GAME_OVER")
    {
        for (int i = 0; i < PERFECT_ONE_GAMER_NUMBER_OPPONENT; ++i)
        {
            unsigned int solution = generate_perfect_diver(player, game);
            Player playerCopy = player;
            population[i].solution = solution;
            population[i].score = evaluate(solution, turns_left, playerCopy, game);
        }
    ...
    for (int i = start; i < OPPONENT_POPULATION_SIZE; ++i)
    {
...
std::array<Solution, POPULATION_SIZE> generate_initial_population(int8_t turns_left, Player &player, Game &game, Player &oppo1, Player &oppo2)
{
    ...
    if (game.hurdle_field != "GAME_OVER")
    {
        for (int i = 0; i < PERFECT_ONE_GAMER_NUMBER; ++i)
        {
            unsigned int solution = generate_perfect_hurdler(player, game);
            Player playerCopy = player;
            population[i].solution = solution;
        ...
        start += PERFECT_ONE_GAMER_NUMBER;
    }
    if (game.diving_sequence != "GAME_OVER")
    {
        for (int i = 0; i < PERFECT_ONE_GAMER_NUMBER; ++i)
...
std::pair<unsigned int, unsigned int> weightedSelectionOppo(const std::array<Solution, OPPONENT_POPULATION_SIZE> &population)
{
    ...
    std::uniform_int_distribution<unsigned int> dis(0, cumulative_score);
    ...
    for (int i = 0; i < OPPONENT_POPULATION_SIZE; ++i)
    {
        current_score += population[i].score;
...
std::array<Solution, OPPONENT_POPULATION_SIZE * 2> generate_next_opponent_generation(const std::array<Solution, OPPONENT_POPULATION_SIZE> &population, u_int16_t cur_gen)
{
    ...
    {
        std::pair<unsigned int, unsigned int> parents = selectionOppo(population);
...
std::array<Solution, POPULATION_SIZE * 2> generate_next_generation(const std::array<Solution, POPULATION_SIZE> &population, u_int16_t cur_gen)
{
    ...
    {
        std::pair<unsigned int, unsigned int> parents = selection(population);
