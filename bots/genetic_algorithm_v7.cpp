...
const double INITIAL_MUTATION_RATE = 0.2;
const double DECAY_FACTOR = 0.01;
const u_int8_t POPULATION_SIZE = 100;
const u_int8_t OPPONENT_POPULATION_SIZE = 20;
...
unsigned int evaluate_compete(unsigned int solution, int8_t turns_left, Player &playerCopy, Game &game, Player &oppo1, Player &oppo2)
{
    ...
    return (hurdle_score + playerCopy.score[0]) * (archery_score + playerCopy.score[1]) * (skating_score + playerCopy.score[2]) * (diving_score + playerCopy.score[3]) * 1000 + (hurdle_score + archery_score + skating_score + diving_score);
}
